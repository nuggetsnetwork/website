import { USER_LOGIN_SUCCESS, USER_LOGIN_FAIL, USER_LOGIN_REQUEST, USER_LOGOUT } from "../constants/userConstants";
import { USER_REGISTER_REQUEST,USER_REGISTER_SUCCESS, USER_REGISTER_FAIL } from "../constants/userConstants";
export const userLoginReducer = (state = { }, action) => {
    switch (action.type) {
        case USER_LOGIN_REQUEST: return ({ loading: true})
        case USER_LOGIN_SUCCESS: return ({ loading: false, userInfo: action.payload })
        case USER_LOGIN_FAIL: return ({error: action.payload })
        case USER_LOGOUT: return ({});
        default: return state;
    }
}

export const userRegisterReducer = (state = {},action) => {
    switch(action.type) {
        case USER_REGISTER_REQUEST: return ({loading: true});
        case USER_REGISTER_SUCCESS: return ({loading: false});
        case USER_REGISTER_FAIL: return ({error: action.payload });
        default: return state;
    }
}